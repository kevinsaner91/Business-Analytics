...
hist(dataset$D5) ### HIER
hist(log(dataset$D5)+1) ### HIER
boxplot(dataset$D5) ### HIER
boxplot(log(dataset$D5)+1) ### HIER

# Quartile für Bowley-skeweness berechnen 

# ((Q3 - Q2) - (Q2 - Q1))/(Q3-A1)
Q1 <- quantile(dataset$D5, prob=c(.5), na.rm=TRUE)  ###  https://www.programmingr.com/statistics/quartile/ 
Q2 <- quantile(dataset$D5, prob=c(.25), na.rm=TRUE)
Q3 <- quantile(dataset$D5, prob=c(.75), na.rm=TRUE)

bowley <- ((Q3 - Q2) - (Q2 - Q1))/(Q3-A1)
bowley
...
# 
# Code:
# shapiro.test(data$rent[which(data$nre=="non‐NRE")])
# shapiro.test(data$rent[which(data$nre=="NRE")])
# #normality is rejected in both groups, a wilcox‐test is appropriate
# wilcox.test(data$rent~data$nre)
# 
# ### R&D person auf zwei gruppen aufgeteilt anschauen -> die inkubatoren und nicht-inkubatoren (Frage: nicht-umfrage-teilnehmer rausfiltenr oder reicht NA-Remove?) Aggregate für die gruppen nutzen

shapiro.test(dataset$D5[which(dataset$D9IncYes1=="No")])
  #dataset_D4_D9IncYes1$D9IncYes1=Responder/Yes/No Responder","Non-Responder"
shapiro.test(dataset$D5[which(dataset$D9IncYes1=="Yes")])

# # YesList <- omit.na(c(dataset$D9IncYes1, exclude = "No,", na.rm=TRUE))
# YesGroup <- aggregate(dataset, FUN="mean", data=dataset$D5, by=("Yes")) # by muss eine List haben
# NoGroup <- aggregate(dataset, FUN="mean", data=dataset$D5, by=("No")) # by muss eine List haben
...
Groups <- aggregate(dataset, FUN="mean", data=dataset$D5, by=list(dataset$D9IncYes1), na.action = na.omit, simplify = TRUE, drop = TRUE) # beim splitt hat es nur noch NA's
Groups <- aggregate(dataset, FUN="mean", by=list(dataset$D9IncYes1), na.action = na.omit) # beim splitt hat es nur noch NA's // FUN macht probleme

wilcox.test(dataset$D5=Yes~dataset$D5=No)
