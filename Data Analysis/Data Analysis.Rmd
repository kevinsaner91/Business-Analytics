...
### 2.4a Graphical distribution of R&D personnel between incubators and non-participants.
...
### 2.4b Graphical distribution of R&D personnel between incubators and non-participants.
#### Bowley Skeweness
...
### Frage an Kevin: Gibt es eine alternative zu "FUN-qqnorm", damit keine Grafik generiert wird?" In der Aufgabe steht, wir sollen aggregate mit qqnorm nutzen. Den Quantile-Quantile Plot brauchen wir jedoch nicht gezeichnet, wir brauchen nur den Wert von test für die Boxplots.
...
boxplot(test$x[1,]$y, test$x[2,]$y, names = c("Non-Participant", "Participant"), outline=FALSE, main="R&D personell distribution")
boxplot(log(test$x[1,]$y)+1, log(test$x[2,]$y)+1, names = c("Non-Participant", "Participant"), outline=FALSE, main="R&D personell distribution with log-numbers")
...
### Frage an Kevin: In der Aufgabe fragen sie zwar nach "normal plots", es macht aber keinen Unterschied zum Boxplot. Vielleicht haben sie geplant hier den QQ-PLot einzusetzen und es einfach normal Plot genannt?
# Normal plot outliers are not drawn
factorvar <- factor(c(dataset$D9IncYes1))
plot(x=factorvar, y=dataset$D5, names = c("Non-Participant", "Participant"), outline=FALSE, main="R&D personell distribution")
plot(x=factorvar, y=log(dataset$D5)+1, names = c("Non-Participant", "Participant"), outline=FALSE, main="R&D personell distribution with log-numbers")
...
### 2.5b Measurement Levels
#### Shapiro-Test

*Calculate the Shapiro-Test to know if the T-Test or the Wilcox-Test is applicable.*

```{r, message=FALSE,warning=FALSE}
  
## Shapiro-Test
# P-value above 0.05 or 5% means it is normally distributed,the Null hypothesis is not rejected and the t-test can be used. Else the Wilcox-test has to be used
...
t.test(test$x[1,]$y, test$x[2,]$y) # H0 auch rejected
...
## Wilcox-Test
# The Wilcox-Test can be used when there is no normal distribution. The Null hypothesis shows at higher than 0.05 or 5%  that both populations are significant differently
